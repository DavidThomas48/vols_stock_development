...
class DamagedStockForm extends \fw\view\form\StdCRUDForm {
    ...
    protected $names       = [];
    protected $parents     = [];
    protected $formname    = "damagedstockform";
    protected $objname     = "Damaged Stock";
    ...
    public function buildinputs($rights=[], $trace=false) {
        ...
        $formfields .= '<div class="vols-movement-header vols-damaged-header">';
        $formfields .= '<span class="vols-movement-icon">&#9888;</span>';
        $formfields .= '<span class="vols-movement-text">Record damaged stock. Select the item, enter the quantity damaged or spoiled, and save.</span>';
        $formfields .= '</div>';
        ...
        $formfields .= $this->component->buildinputrow("qty",      3, "", 'Quantity damaged', 'Number of units damaged or spoiled', 10, 10, true, '', '');
    ...
    public function formscript() {
        ...
        $script = $this->vols_masterscript(
            $this->formname,
            $this->objname,
            true,   // idselection — show dropdown of existing damaged stock records
            false,  // adjustnamerow
            true,   // updatefields
            false,  // inclmulti
            '',     // postajaxscript
