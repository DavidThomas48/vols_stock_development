...
class DeliveryForm extends \fw\view\form\StdCRUDForm {
    ...
    protected $names       = [];
    protected $parents     = [];
    protected $formname    = "deliveryform";
    protected $objname     = "Delivery";
    protected $movementid;

    public function __construct(protected FormComponent $component) {
        $this->singlerecord = false;
        // Only allow adding new deliveries — no edit or delete of historical records
    ...
    public function buildinputs($rights=[], $trace=false) {
        ...
        $formfields .= '<div class="vols-movement-header vols-delivery-header">';
        $formfields .= '<span class="vols-movement-icon">&#8679;</span>';
        $formfields .= '<span class="vols-movement-text">Record stock received. Select the item, enter the quantity delivered, and save.</span>';
        $formfields .= '</div>';
        ...
        $formfields .= $this->component->buildinputrow("qty",      3, "", 'Quantity',  'Number of units received', 10, 10, true, '', '');
    ...
    public function formscript() {
        ...
        $script = $this->vols_masterscript(
            $this->formname,
            $this->objname,
            true,   // idselection — show dropdown of existing deliveries
            false,  // adjustnamerow — no "name" field to auto-update in dropdown
            true,   // updatefields
            false,  // inclmulti
            '',     // postajaxscript
            '',     // postupdatescript
